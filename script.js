(() => {
  ...
  function redeemCode(){const input=$("#redeemInput"),code=input.value.trim().toUpperCase(),rewards={ADI7S:20000,SOMU34:34,SOMU:149};if(!code){setShopMessage("Enter a redeem code.");return;}if(!(code in rewards)){setShopMessage("Invalid redeem code.");return;}const key=`flowerHuntCode_${code}`;if(localStorage.getItem(key)==="1"){setShopMessage("This code was already redeemed on this device.");return;}state.diamonds+=rewards[code];localStorage.setItem(key,"1");saveEconomy();updateShopUI();input.value="";setShopMessage(`Success! +${rewards[code].toLocaleString()} 💎 added.`);tone(880,.08);setTimeout(()=>tone(1100,.1),70);}
  ...
  async function shareImage(){
    ...
    const shareText=`*https://drive.google.com/file/d/19TP7Qkjw11evq2X1h-BL4OBPDDsbxuif/view?usp=drivesdk*

*Download, Play and Beat My Score*`;
    if(navigator.share){
      try{
        if(navigator.canShare?.({files:[file]})){await navigator.share({title:"Flower Hunt Score",text:shareText,files:[file]});quitToMenu();return;}
        await navigator.share({title:"Flower Hunt Score",text:shareText});quitToMenu();return;
      }catch(e){if(e?.name==="AbortError"){quitToMenu();return;}throw e;}
    }
    const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="flower-hunt-score.png";a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1200);quitToMenu();
  }
  ...
  $("#shareNowBtn").onclick=shareFromCard;$("#downloadShareBtn").onclick=downloadShareCard;$("#closeShareBtn").onclick=quitToMenu;
