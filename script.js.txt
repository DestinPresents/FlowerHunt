(() => {
  ...
  function chooseObject(){
    ...
    // Better balanced spawn probabilities:
    // TIME: 20% bomb, 10% bonus clock, 70% flower.
    if(state.mode==="time"){
      if(r < 0.20) return "bomb";
      if(r < 0.30) return "clock";
      return "flower";
    }

    // SURVIVAL: starts at 15% bomb chance and gradually increases to 30%.
    if(state.mode==="survival"){
      const bombChance = Math.min(0.30, 0.15 + state.score / 3000);
      return r < bombChance ? "bomb" : "flower";
    }

    // CHALLENGE: starts at 12% and gradually increases to 22%.
    const bombChance = Math.min(0.22, 0.12 + state.score / 4000);
    return r < bombChance ? "bomb" : "flower";
  }
  ...
  function spawnObject(){
    ...
    const obj=hole.querySelector(".object"); obj.className=`object ${type}`; obj.textContent=type==="flower"?flowerEmoji():type==="bomb"?"💣":type==="clock"?"⏱️":""; hole.classList.remove("hit","bomb-hit");void hole.offsetWidth;hole.classList.add("active");
