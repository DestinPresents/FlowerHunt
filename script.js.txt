(() => {
  ...
  function updateHome() {
    ...
    updateProgressUI(); updateLastRunCard(); updateThemeLocks(); applyTheme(state.selectedTheme, false);
  }
  ...
  function applyTheme(theme, save=true){
    ...
    $$(".theme-dot").forEach(b=>b.classList.toggle("active",b.dataset.theme===theme));
    $("#themeNameHome").textContent=info.name;
  }
  function updateThemeLocks(){
    const owned=ownedThemes();
    $$(".theme-dot").forEach(btn=>{ const free=btn.dataset.theme==="sunny", isOwned=owned.includes(btn.dataset.theme); btn.classList.toggle("locked",!free&&!isOwned); });
  ...
  function buyTheme(theme){
    ...
    const owned=ownedThemes(); if(owned.includes(theme)) return applyTheme(theme);
    if(state.diamonds<99){ popMessage("💎 Need 99 diamonds"); return; }
  ...
  function updateShopUI(){
    $("#shopDiamonds").textContent=state.diamonds; $("#slowOwned").textContent=state.slowOwned; $("#doubleOwned").textContent=state.doubleOwned;
  ...
  $$(".theme-dot").forEach(b=>b.addEventListener("click",()=>buyTheme(b.dataset.theme)));
